...
function ErrorToast(props: IErrorMessage) {
  return <Alert severity="error" sx={{position: "fixed", bottom: "0", width: "100vw" }}>[{props.status}] {props.message}</Alert>;
}
  ...
  return (
    <Fragment>
      {ReactDOM.createPortal(
        <ErrorToast message={props.message} status={props.status} />,
